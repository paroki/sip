...
export class Router {
  ...
  generate (name: string, opt_params?: RouteParams, absolute?: boolean): string {
    ...
    let optional = true
    let host = ''
    const port = (typeof this.getPort() === 'undefined' || this.getPort() === null) ? '' : this.getPort()

    route.tokens.forEach((token) => {
      if (token[0] === 'text' && typeof token[1] === 'string') {
      ...
      if (token[0] === 'variable') {
        ...
        const hasDefault = route.defaults && !Array.isArray(route.defaults) && typeof token[3] === 'string' && (token[3] in route.defaults)
        if (optional === false || !hasDefault || ((typeof token[3] === 'string' && token[3] in params) && !Array.isArray(route.defaults) && params[token[3]] != route.defaults[token[3]])) {
          let value
    ...
    if (route.requirements && ('_scheme' in route.requirements) && this.getScheme() != route.requirements._scheme) {
      ...
      url = route.requirements._scheme + '://' + currentHost + (currentHost.includes(':' + port) || port === '' ? '' : ':' + port) + url
    } else if (typeof route.schemes !== 'undefined' && typeof route.schemes[0] !== 'undefined' && this.getScheme() !== route.schemes[0]) {
      const currentHost = host || this.getHost()

      url = route.schemes[0] + '://' + currentHost + (currentHost.includes(':' + port) || port === '' ? '' : ':' + port) + url
    } else if (host && this.getHost() !== host + (host.includes(':' + port) || port === '' ? '' : ':' + port)) {
      url = this.getScheme() + '://' + host + (host.includes(':' + port) || port === '' ? '' : ':' + port) + url
    } else if (absolute === true) {
      url = this.getScheme() + '://' + this.getHost() + (this.getHost().includes(':' + port) || port === '' ? '' : ':' + port) + url
    }
